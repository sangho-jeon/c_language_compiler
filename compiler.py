...
def non_char_token(token, value, index, raw_code):
    ...
    if char == ';':
        ...
        if 48 <= ord(raw_code[index+1]) <= 57:
            print("Lexical error!! float number must start with digit!", index)
        return int(index + 1)
    elif char == '<':
        if raw_code[index+1] == '<':
            token.append("B_O")
...
def string_token(token, value, index, raw_code, ascii):
    ...
    while ended:
        try:
            ...
            print("Lexical error!!", chr(34), "expected!!", index)
            return int(tmp + 2)
...
def numeric_token(token, value, index, raw_code, ascii):
    ...
    if raw_code[tmp] == "0":
        ...
        if ord(raw_code[tmp + 1]) == 46:
            ...
            print("Lexical error!! number cannot start with 0!!", index)
            return int(tmp + 1)
    ended = False
    while not ended:
        block.append(raw_code[tmp])
        if ord(raw_code[tmp + 1]) == 46 and dot == False:
            ...
            print("Lexical error!! float point number must have one dot!!", index)
            return int(tmp + 1)
        elif ord(raw_code[tmp+1]) != 46 and (ord(raw_code[tmp+1]) <=47 or ord(raw_code[tmp+1]) >= 58):
            ended = True
        else:
    ...
    if dot == True and block[-1] == "0" and ord(block[-2]) != 46:
        print("Lexical error!! float must end with non zero digit!!", index)
        return int(tmp+1)
    if dot == True:
        token.append("FLOAT")
        value.append(chunk)
